...
/*   Updated: 2024/01/08 18:35:14 by phantasiae       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
pthread_mutex_t *init_forks(t_data *data)
{
	...
	i=-1;
	data->fork_locks=malloc(sizeof(pthread_mutex_t)*(data->number_of_philosophers));
	if(!data->fork_locks)
	{
		free(data->philo);
		return(NULL);
...
t_philo	*init_philo(t_data *data)
{
	...
	while(++i<data->number_of_philosophers)
	{
		...
		data->philo[i].fork[0]=i+1;
		data->philo[i].fork[1]=i+2;//CHANGE THIS 
		data->philo[i].mealcount=0;
		data->philo[i].status=1;
		if (pthread_create(&data->philo[i].thread_id, NULL, &thread_routine, &data)!=0)
		{
			free(data->philo);
			return(NULL);
		}
